...
import { CredentialsManager, SupportedPlatform } from '../../player/credentials-manager';
...
const allowedPlatforms: SupportedPlatform[] = ['youtube', 'dailymotion', 'soultv'];

function validatePlatform(value: string): SupportedPlatform | null {
  if ((allowedPlatforms as string[]).includes(value)) return value as SupportedPlatform;
  return null;
}
...
export function createCredentialsRouter(): Router {
  ...
  router.get('/', (_request, response) => {
    const rows = getDb()
      .prepare('SELECT id, platform, type, label, value, active, is_default, created_at FROM credentials ORDER BY type, platform, id')
      .all();
    response.json(rows);
  });

  router.post('/cookie/:platform', upload.single('file'), (request, response) => {
    ...
    getDb().prepare('DELETE FROM credentials WHERE platform = ? AND type = ?').run(platform, 'cookie');
    getDb()
      .prepare('INSERT INTO credentials(platform, type, label, value, active, is_default) VALUES (?, ?, ?, ?, 1, 1)')
  ...
  router.delete('/cookie/:platform', (request, response) => {
    ...
    const row = getDb()
      .prepare('SELECT value FROM credentials WHERE platform = ? AND type = ? LIMIT 1')
  ...
  router.patch('/cookie/:platform/toggle', (request, response) => {
    ...
    const current = getDb()
      .prepare('SELECT active FROM credentials WHERE platform = ? AND type = ? LIMIT 1')
    ...
    getDb().prepare('UPDATE credentials SET active = ? WHERE platform = ? AND type = ?').run(next, platform, 'cookie');
  ...
  router.post('/ua', (request, response) => {
    ...
    const resolvedPlatform = platform && validatePlatform(platform) ? platform : 'global';

    const info = getDb()
      .prepare('INSERT INTO credentials(platform, type, label, value, active, is_default) VALUES (?, ?, ?, ?, 1, 0)')
      .run(resolvedPlatform, 'user-agent', label ?? userAgent.slice(0, 42), userAgent);
    response.json({ ok: true, id: info.lastInsertRowid });
  });

  router.delete('/ua/:id', (request, response) => {
    const id = Number(request.params.id);
    getDb().prepare('DELETE FROM credentials WHERE id = ? AND type = ?').run(id, 'user-agent');
    response.json({ ok: true });
  });
  ...
  router.patch('/ua/:id/default', (request, response) => {
    const id = Number(request.params.id);
    const current = getDb()
      .prepare('SELECT id, platform, type FROM credentials WHERE id = ? AND type = ?')
      .get(id, 'user-agent') as { id: number; platform: string; type: string } | undefined;
    ...
    getDb().prepare('UPDATE credentials SET is_default = 0 WHERE type = ? AND platform = ?').run('user-agent', current.platform);
  ...
  router.post('/test', async (request, response) => {
    ...
    if (tool === 'streamlink') {
      const ok = await streamlinkHasPlayableStream(url, creds.userAgent, creds.cookieFile);
      response.json({ success: ok, output: ok ? 'stream resolvida' : 'stream não resolvida' });
      return;
    }
